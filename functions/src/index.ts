...
export const beforeSignIn = functions.auth.user().beforeSignIn(async (user, context) => {
  ...
  if (!allowlistDoc.exists) {
    ...
    throw new functions.auth.HttpsError(
      "permission-denied",
      "Access denied. Your account is not authorized to use this app. Contact the administrator for access."
    );
  }
...
export const addToAllowlist = functions.https.onCall(async (data, context) => {
  // Only allow if caller has admin role
  if (!context.auth || context.auth.token.role !== "admin") {
    throw new functions.https.HttpsError(
      "permission-denied",
      "Only admins can add users to the allowlist"
    );
  }
...
export const removeFromAllowlist = functions.https.onCall(async (data, context) => {
  if (!context.auth || context.auth.token.role !== "admin") {
    throw new functions.https.HttpsError(
      "permission-denied",
      "Only admins can remove users from the allowlist"
    );
  }
...
export const listAllowedUsers = functions.https.onCall(async (data, context) => {
  if (!context.auth || context.auth.token.role !== "admin") {
    throw new functions.https.HttpsError(
      "permission-denied",
      "Only admins can list allowed users"
    );
  }
